...
struct MasterData {
    ...
    int8_t player = 0; // 1b номер игрока, которой фальшстартанул/отвечает

    uint32_t startSend = 0; // 4b
    uint32_t endSend = 0; // 4b
    uint8_t error = 0; // 1b
    uint8_t isData = 0; // 1b
    uint8_t needDrop = 0; // 1b Нужно ли сбрасывать состояние кнопки
    uint8_t _nop[18];
};

struct SlaveData {
    uint32_t slaveTime = 0; // 4b время на slave, выставляется автоматически
...
const char pipeMaster[6] = PIPE_MASTER;

const char pipeSlave[4][6] = {
